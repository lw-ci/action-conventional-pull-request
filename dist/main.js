...
export async function run() {
    try {
        ...
        if (!prContext) {
            throw new Error('`pull_request_target` or `pull_request` events are required to execute this action.');
        }
        ...
        if (ignoreLabels) {
            ...
            for (const labelName of labelNames) {
                if (ignoreLabels.includes(labelName)) {
                    core.info(`Validation was skipped because the PR label "${labelName}" was found.`);
                    return;
                }
            }
        }
        ...
        if (!report.valid) {
            throw new Error(`The following pull request title failed validation: "${report.input}"`);
        }
    }
    catch (error) {
